...
@section('title', 'List Onduty')

@push('styles')
    <link rel="stylesheet" href="{{ asset('assets/css/onduty_page.css') }}">
@endpush
@php
    $maintenance_list_onduty = false; 
@endphp
@section('content')
    @if($maintenance_list_onduty && !View::hasSection('hide_maintenance'))
                ...
                <table class="table table-bordered table-hover-custom align-items-center">
                    ...
                    <tbody>
                        @forelse ($onDutyList as $att)
                            @php
                                $discordId = $att->user->employee->discord_id ?? null;
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center">Không có ai đang On-Duty.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
